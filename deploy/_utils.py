...
def convert_response_to_json(response: str) -> dict:
    ...
    if response is None or not isinstance(response, str):
        logger.error("无效的输入: response 必须是字符串")
...
def limit_async_func_call(max_size: int, waitting_time: float = 0.0001):
    ...
    - waitting_time (float): 当并发调用数达到上限时，等待的时间间隔（单位：秒），默认值为 0.0001 秒。

    返回：
    - 一个装饰器，用于限制异步函数的并发调用次数。
    """
    def final_decro(func):
        """Not using async.Semaphore to aovid use nest-asyncio"""
        """
        装饰器内部函数，应用对被装饰的异步函数的限制。
        注意：这里没有使用 `asyncio.Semaphore`，以避免 `nest-asyncio` 相关问题。

        参数：
        - func (coroutine function): 需要被限制的异步函数。
        ...
        async def wait_func(*args, **kwargs):
            ...
            当并发调用数超过 `max_size` 时，会等待 `waitting_time` 后重试。
            ...
            - `func` 的执行结果。
            """
            nonlocal __current_size # 使用 `nonlocal` 关键字修改 `__current_size` 变量
            # 如果当前执行的异步调用数达到最大值，则等待
